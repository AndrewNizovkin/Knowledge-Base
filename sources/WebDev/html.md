...
<body>
  <form>
    <!-- другие элементы -->
    <label for="code">Цифровой код</label>
    <input inputmode="numeric" type="text" id="code">
    <!-- другие элементы -->
  </form>
</body>
```

Атрибут `inputmode` решает только одну задачу, а именно сообщает, какую виртуальную клавиатуру показывать браузерам при вводе данных на мобильных платформах. Для отображения виртуальной клавиатуры только с цифрами подходит значение `numeric`
...
В примере будет подставлена строка «Заказ №5241836».
